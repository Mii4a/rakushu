...
3. `.env.local` に値を設定
   ...
   ```bash
   GOOGLE_MAPS_SERVER_API_KEY=replace-with-google-maps-server-key
   NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_API_KEY=replace-with-google-maps-browser-key
   STRIPE_SECRET_KEY=replace-with-stripe-test-secret-key
   STRIPE_WEBHOOK_SECRET=replace-with-stripe-webhook-secret
   STRIPE_PRICE_STARTER=replace-with-stripe-starter-price-id
   STRIPE_PRICE_PLUS=replace-with-stripe-plus-price-id
   STRIPE_PRICE_PRO=replace-with-stripe-pro-price-id
