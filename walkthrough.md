...
- さらに `src` 全体を再探索して残っていた `db.query` を actions / API / edit page / webhook helper から除去し、Edge で危ない read path を repo 内から取り切った。
- 追加 hardening 後もローカルでは `npm test` と `npm run build` が通ったので、本番反映後に同じ Playwright smoke を再実行して確認する。
