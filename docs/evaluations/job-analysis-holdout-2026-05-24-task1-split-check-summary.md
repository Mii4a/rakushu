...
- reviewer: taro-bot
- scorecard_path: docs/evaluations/job-analysis-holdout-2026-05-24-task1-split-check-scorecard.csv
- notes_path: docs/evaluations/job-analysis-holdout-2026-05-24-task1-split-check-notes.md
...
- scope_note: 50 approved holdout raw texts re-parsed with current parser v1.6.1; rows with historical analysis_id keep observed feedback_saved from manifest while public-holdout rows use current shouldCreateFeedback simulation
...
- same failure type recurring 3+ times in same shape: job_board_detail:salary_text_without_base_salary=5, noisy_promo:too_many_unknown_critical_fields=5, prose_heavy:too_many_unknown_critical_fields=5, company_careers:too_many_unknown_critical_fields=4, prose_heavy:benefits_suspected_but_not_extracted=3
...
- feedback evidence split:
  ...
  - current simulated all-row recall: 24/44 (55%)

## Regression safety
- high-signal fixtures from feedback: 23 current anonymized fixture texts in `fixtures/jobs/`
...
## Conclusion
- final decision: FAIL
- biggest remaining gap: job_board_detail:salary_text_without_base_salary recurring 5 times
