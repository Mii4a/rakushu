...
export async function generateResumeDraftAction(_: ResumeActionState, formData: FormData): Promise<ResumeActionState> {
  const user = await requireUser();
  const plan = await getUserPlan(user.id);

  if (plan !== "pro") {
    return { error: "履歴書自動作成はProプラン限定機能です。", result: null };
  }
  ...
  if (!parsed.success) {
    return { error: parsed.error.issues[0]?.message ?? "入力値が不正です", result: null };
  }

  const d = parsed.data;
  const content = `【履歴書（${d.templateName}）】\n\n氏名: ${d.fullName}\nふりがな: ${d.furigana || ""}\n電話番号: ${d.phone || ""}\nメール: ${d.email || ""}\n\n--- 学歴 ---\n${d.education}\n\n--- 職歴・経験 ---\n${d.experience || "(未記入)"}\n\n--- 自己PR ---\n${d.selfPr}\n\n--- 志望動機 ---\n${d.motivation}\n`;
